...
function writeDelugeDaemon2Debug()
{
cat > /etc/init.d/deluge-daemon <<'Endofmessage'
#!/bin/sh
### BEGIN INIT INFO
...
NAME2="deluge"
DAEMON1=/usr/bin/deluged
DAEMON1_ARGS="-d -L debug -l /var/log/deluge/daemon/debug.log"         # none, critical, error, warning, info, debug
DAEMON2=/usr/bin/deluge-web
DAEMON2_ARGS="-L debug -l /var/log/deluge/web/debug.log"               # none, critical, error, warning, info, debug
PIDFILE1=/var/run/$NAME1.pid
PIDFILE2=/var/run/$NAME2.pid
UMASK=0                       # Change this to 0 if running deluged as its own user, org 022
PKGNAME=deluge-daemon
...
function apt_deluge_original_setup()
{
	...
	apt-get install -y python python-twisted python-twisted-web python-openssl python-simplejson \
...
function apt_deluge_master_setup()
{
	...
	# install add. needed packages (compil libtorrent eg.
	# libboost --- libboost-system-dev libboost-filesystem-dev libboost-thread-dev
	apt-get install -y subversion gcc locate g++ build-essential libboost-all-dev
}
...
function main_newinstall_deluge_stable_org()
{
	clear 
	# remove older Version without extra step from menu
	first_deluge_remove
...
function main_update_stable()
{
	...
	# finish the script
	myipaddress=$(hostname -I | tr -d ' ')
	dialog --backtitle "urbanswelt.de - DelugePi Setup." --msgbox "If everything went right, Deluge should now be available again at the URL http://$myipaddress:$__delugeport." 20 60    
}
